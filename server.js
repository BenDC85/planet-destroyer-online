...
const SV_PLANET_GRAVITY_CORE_RADIUS_FACTOR = 0.20; // NEW: For "soft center" gravity
...
const SV_PROJECTILE_DEFAULT_MASS_KG = 50; // NEW: Authoritative projectile mass
...
const SV_SHIP_RADIUS_PX = 12.5; // KEPT THIS AT 12.5 (RADIUS). Client must match.
...
// REPLACED with superior logic from client, adapted for server
function serverFindAccurateImpactPoint(p1, p2, planet) {
    ...
    const p1IsInsideSolid = serverIsPointInsideCircle(p1.x, p1.y, planet.x, planet.y, planet.originalRadius) &&
                           !serverIsPointInsideAnyCrater(p1.x, p1.y, planet);
    if (p1IsInsideSolid) {
        return { t: 0, point: { x: p1.x, y: p1.y } };
    }
    ...
    for (let i = 0; i < SV_MAX_PLAYER_SPAWN_ATTEMPTS; i++) {
        ...
        for (const planet of serverPlanetsState) {
            ...
            if (planet.isBlackHole) {
                ...
                planetEffectiveRadius = planet.originalRadius; // CHANGED to originalRadius
                requiredDistance = planetEffectiveRadius + safetyBufferFromPlanet;
            }
    ...
    for (let i = 0; i < numPlanets; i++) {
        ...
        while (attempts < SV_MAX_PLANET_SPAWN_ATTEMPTS_PER_PLANET && !placed) {
            ...
            for (const existingPlanet of serverPlanetsState) {
                ...
                const requiredSeparation = (radius_pixels + existingPlanet.originalRadius) * SV_MIN_PLANET_SEPARATION_FACTOR; // Use originalRadius
            ...
            if (!overlaps) {
                ...
                serverPlanetsState.push({
                    id: `srv_planet_${nextServerPlanetId++}`, type: pTypeKey, x, y, radius: radius_pixels, originalRadius: radius_pixels,
                    radius_m: radius_m, originalRadius_m: radius_m, // ADDED originalRadius_m
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (joinData) => {
        ...
        if (Object.keys(players).length === 0) { playerNumberAssigner = 0; } // Simplified player number assignment
    ...
    socket.on('request_fire_projectile', (projectileData) => {
        ...
        // REMOVED check for massKg from client. It will not be trusted.
        if (!projectileData || typeof projectileData.startX !== 'number' || typeof projectileData.startY !== 'number') {
            console.warn(`[SERVER] Invalid projectile data from ${player.playerName} (${socket.id})`); return;
        }
        ...
        const newServerProjectile = {
            id: serverProjId, ownerShipId: player.socketId,
            x: projectileData.startX, y: projectileData.startY,
            prevX: projectileData.startX, prevY: projectileData.startY,
            vx: vx, vy: vy, angle: projectileData.angle,
            initialSpeedInternalPxFrame: initialSpeed,
            massKg: SV_PROJECTILE_DEFAULT_MASS_KG, // USE AUTHORITATIVE MASS
            isActive: true, framesAlive: 0,
        };
    ...
    for (let i = serverProjectiles.length - 1; i >= 0; i--) {
        ...
        if (proj.isActive) {
            for (const playerId in players) {
                ...
                const collisionDist = SV_SHIP_RADIUS_PX + (SV_PROJECTILE_SIZE_PX / 2); // Use radius
        ...
        if (proj.isActive) {
            for (const planet of serverPlanetsState) {
                ...
                const planetBounds = { minX: planet.x - planet.originalRadius, maxX: planet.x + planet.originalRadius, minY: planet.y - planet.originalRadius, maxY: planet.y + planet.originalRadius, }; // Use originalRadius
                if (!(projMaxX < planetBounds.minX || projMinX > planetBounds.maxX || projMaxY < planetBounds.minY || projMinY > planetBounds.maxY)) {
                    ...
                    if (impact) {
                        ...
                        const massBeforeImpact = planet.massKg; const radiusMBeforeImpact = planet.originalRadius_m; // Use originalRadius_m
                        const bindingEnergyBeforeImpact = serverCalculateBindingEnergy(massBeforeImpact, radiusMBeforeImpact); const cumulativeEnergyAfterThisImpact = (planet.cumulativeImpactEnergy || 0) + impactKE;
                        ...
                        // REMOVED "liquid drop" radius calculation
                        
                        planet.radius = planet.originalRadius; // Radius property now stays the same as original
...
function updateServerChunks() {
    ...
    for (let i = serverChunks.length - 1; i >= 0; i--) {
        ...
        serverPlanetsState.forEach(planet => {
            ...
            } else if (planet.massKg > 0) { // MODIFIED: Simplified to not require radius_m
                const effectiveG = SV_G * SV_PLANET_GRAVITY_MULTIPLIER_INTERNAL;
                const M = planet.massKg;
                const coreRadius_m = (planet.originalRadius_m || (planet.originalRadius / SV_PIXELS_PER_METER)) * SV_PLANET_GRAVITY_CORE_RADIUS_FACTOR;
        ...
        if (chunk.isActive) {
            for (const planet of serverPlanetsState) {
                ...
                if (impact) {
                    ...
                    const radiusMBeforeImpact = planet.originalRadius_m; // Use originalRadius_m
        ...
        if (chunk.isActive) {
            for (const playerId in players) {
                const player = players[playerId];
                if (!player.isAlive) continue;
                const collisionDist = SV_SHIP_RADIUS_PX + (chunk.size || SV_CHUNK_DEFAULT_BASE_SIZE_MIN); // Use radius
                const distSqToShip = serverDistanceSq({x: chunk.x, y: chunk.y}, player);

                if (distSqToShip < collisionDist * collisionDist) {
                    chunk.isActive = false;

                    // CORRECTED KE Calculation
