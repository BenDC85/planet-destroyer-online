...
const SV_MAX_PLANET_SPAWN_ATTEMPTS = 50;
const SV_REFERENCE_DENSITY_CONFIG = 5515; 
const SV_REFERENCE_YIELD_STRENGTH_CONFIG = 1e8; 
const SV_PLANET_TYPES = {
    icy: { density: 1500, yieldStrength: 1e6, baseColorRGB_server: [173, 216, 230], clientColor: '#ADD8E6' },
    rocky: { density: SV_REFERENCE_DENSITY_CONFIG, yieldStrength: SV_REFERENCE_YIELD_STRENGTH_CONFIG, baseColorRGB_server: [160, 82, 45], clientColor: '#A0522D' },
    metallic: { density: 8000, yieldStrength: 5e8, baseColorRGB_server: [169, 169, 169], clientColor: '#A9A9A9' }
};
...
const SV_PROJECTILE_MAX_LIFESPAN_FRAMES = 10 * SV_PROJECTILE_SIMULATION_FPS; 
...
const SV_CHUNK_LIFESPAN_FRAMES = 400; 
const SV_CHUNK_DEFAULT_BASE_SIZE_MIN = 8;
const SV_CHUNK_DEFAULT_BASE_SIZE_RANDOM_RANGE = 12;
...
const SV_CHUNK_UPDATE_INTERVAL_MS = 100; 
...
function generatePlanetsOnServer(numPlanetsOverride = null) { 
    ...
    const forcedPlanetSetup = [ { type: 'icy', sizeCategory: 'medium' }, { type: 'icy', sizeCategory: 'medium' }, { type: 'rocky', sizeCategory: 'medium' }, { type: 'rocky', sizeCategory: 'medium' }, { type: 'metallic', sizeCategory: 'medium' }, { type: 'metallic', sizeCategory: 'medium' }, ];
    const sizeCategoryRadii = { small: { min: SV_MIN_PLANET_RADIUS_RANGE_PX, max: SV_MIN_PLANET_RADIUS_RANGE_PX + (SV_MAX_PLANET_RADIUS_RANGE_PX - SV_MIN_PLANET_RADIUS_RANGE_PX) * 0.33 }, medium: { min: SV_MIN_PLANET_RADIUS_RANGE_PX + (SV_MAX_PLANET_RADIUS_RANGE_PX - SV_MIN_PLANET_RADIUS_RANGE_PX) * 0.33, max: SV_MIN_PLANET_RADIUS_RANGE_PX + (SV_MAX_PLANET_RADIUS_RANGE_PX - SV_MIN_PLANET_RADIUS_RANGE_PX) * 0.66 }, large: { min: SV_MIN_PLANET_RADIUS_RANGE_PX + (SV_MAX_PLANET_RADIUS_RANGE_PX - SV_MIN_PLANET_RADIUS_RANGE_PX) * 0.66, max: SV_MAX_PLANET_RADIUS_RANGE_PX } };
    const biasCenterX = SV_TARGET_CENTER_OFFSET_X; const biasCenterY = SV_TARGET_CENTER_OFFSET_Y; const BIAS_CANDIDATES = 3; const planetTypesAvailable = Object.keys(SV_PLANET_TYPES);
    for (let i = 0; i < numPlanets; i++) {
        let placed = false; let attempts = 0;
        while (!placed && attempts < SV_MAX_PLANET_SPAWN_ATTEMPTS) {
            attempts++; let planetTypeKey, radius_pixels;
            if (numPlanets === 6 && i < forcedPlanetSetup.length) { const setup = forcedPlanetSetup[i]; planetTypeKey = setup.type; const radiusRange = sizeCategoryRadii[setup.sizeCategory] || sizeCategoryRadii.medium; radius_pixels = radiusRange.min + Math.random() * (radiusRange.max - radiusRange.min); } else { planetTypeKey = planetTypesAvailable[Math.floor(Math.random() * planetTypesAvailable.length)]; radius_pixels = SV_MIN_PLANET_RADIUS_RANGE_PX + Math.random() * (SV_MAX_PLANET_RADIUS_RANGE_PX - SV_MIN_PLANET_RADIUS_RANGE_PX); }
            const radius_m = radius_pixels / SV_PIXELS_PER_METER; const borderX = radius_pixels * SV_PLANET_SPAWN_BORDER_FACTOR; const borderY = radius_pixels * SV_PLANET_SPAWN_BORDER_FACTOR; const spawnMinX = SV_WORLD_MIN_X + borderX; const spawnMaxX = SV_WORLD_MAX_X - borderX; const spawnMinY = SV_WORLD_MIN_Y + borderY; const spawnMaxY = SV_WORLD_MAX_Y - borderY;
            if (spawnMaxX <= spawnMinX || spawnMaxY <= spawnMinY) continue;
            let bestX = 0, bestY = 0; let minBiasDistSq = Infinity;
            for (let k = 0; k < BIAS_CANDIDATES; k++) { const currentX = spawnMinX + Math.random() * (spawnMaxX - spawnMinX); const currentY = spawnMinY + Math.random() * (spawnMaxY - spawnMinY); const distSqToBiasCenter = serverDistanceSq({x: currentX, y: currentY}, {x: biasCenterX, y: biasCenterY}); if (k === 0 || distSqToBiasCenter < minBiasDistSq) { minBiasDistSq = distSqToBiasCenter; bestX = currentX; bestY = currentY; } }
            let x = bestX; let y = bestY; let overlap = false;
            for (const p of serverPlanetsState) { const minDist_pixels = (radius_pixels + p.radius) * SV_MIN_PLANET_SEPARATION_FACTOR; if (serverDistanceSq({ x, y }, { x: p.x, y: p.y }) < minDist_pixels * minDist_pixels) { overlap = true; break; } }
            if (!overlap) {
                const pTypeData = SV_PLANET_TYPES[planetTypeKey] || SV_PLANET_TYPES[SV_DEFAULT_PLANET_TYPE_KEY]; const volume_m3 = (4/3) * Math.PI * Math.pow(radius_m, 3); const initialMassKg = volume_m3 * pTypeData.density; if (initialMassKg <= 0 || !isFinite(initialMassKg)) continue;
                serverPlanetsState.push({ id: `srv_planet_${nextServerPlanetId++}`, type: planetTypeKey, x, y, radius: radius_pixels, originalRadius: radius_pixels, radius_m, massKg: initialMassKg, originalMassKg: initialMassKg, color: pTypeData.clientColor, density: pTypeData.density, yieldStrength: pTypeData.yieldStrength, craters: [], isBreakingUp: false, breakupFrame: 0, lastImpactPoint: null, isDestroying: false, willBecomeBlackHole: false, isBlackHole: false, isDestroyed: false, explosionFrame: 0, destructionElapsedTime: 0, chunkGracePeriodFrame: 0, shockwave2ReversalStartRadius: null, spinDirection: null, spinMagnitude: 0, bhParticlesSpawnedCount: 0, cumulativeImpactEnergy: 0, cumulativeCraterAreaEffect: 0, originalSurfaceArea: Math.PI * radius_pixels * radius_pixels });
                placed = true;
            }
        }
        if (!placed) console.warn(`Server failed to place planet ${i+1}.`);
    }
    console.log(`Server generated ${serverPlanetsState.length} planets.`);
}
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (joinData) => { 
        ...
        playerNumberAssigner++; if (playerNumberAssigner > MAX_PLAYERS || Object.keys(players).length === 0) playerNumberAssigner = 1;  
        const initialX = (playerNumberAssigner - 1) * SPAWN_OFFSET_X - ((MAX_PLAYERS-1)*SPAWN_OFFSET_X / 2); const initialY = (playerNumberAssigner - 1) * SPAWN_OFFSET_Y - ((MAX_PLAYERS-1)*SPAWN_OFFSET_Y / 2); const initialAngle = -Math.PI / 2;
        players[socket.id] = { socketId: socket.id, playerNumber: playerNumberAssigner, userId: trimmedUserId, playerName: trimmedPlayerName, shipColor: shipColor, x: initialX, y: initialY, angle: initialAngle, health: SV_SHIP_DEFAULT_HEALTH, isAlive: true, lastFireTime: 0  };
        console.log(`Socket ${socket.id} joined as Player ${players[socket.id].playerNumber} (Name: \"${trimmedPlayerName}\", HP: ${players[socket.id].health})`);
    ...
    socket.on('request_world_reset', (clientSuggestedSettings) => {
        ...
        generatePlanetsOnServer(numPlanetsForReset); serverProjectiles = []; nextProjectileId = 0; serverChunks = []; nextServerChunkId = 0;
        Object.values(players).forEach((player, index) => {
            player.x = (index) * SPAWN_OFFSET_X - ((Object.keys(players).length-1)*SPAWN_OFFSET_X / 2); player.y = (index) * SPAWN_OFFSET_Y - ((Object.keys(players).length-1)*SPAWN_OFFSET_Y / 2); player.angle = -Math.PI / 2; player.lastFireTime = 0;
            player.health = SV_SHIP_DEFAULT_HEALTH; player.isAlive = true; // Reset health on world reset
...
function generateServerChunks(planet, impactPoint) { /* ... as before ... */ }
// (Copying generateServerChunks to save space)
...
function updateServerProjectiles() { 
    ...
    for (let i = serverProjectiles.length - 1; i >= 0; i--) {
        ...
        serverPlanetsState.forEach(planet => { /* ... gravity and BH absorption ... */ 
                ...
                if (distSqToShip < collisionDist * collisionDist) {
                    ...
                    if (player.health <= 0) {
                        ...
                        io.emit('ship_destroyed', { 
                            destroyedShipId: player.socketId, 
                            destroyedShipName: player.playerName,
                            killerId: proj.ownerShipId,
                            killerName: killerName
                        });
                        
                        setTimeout(() => {
                            if (players[player.socketId]) { 
                                players[player.socketId].isAlive = true;
                                players[player.socketId].health = SV_SHIP_DEFAULT_HEALTH;
                                const playerNumberForSpawn = players[player.socketId].playerNumber; // Use stored player number
                                const activePlayerCount = Object.keys(players).length; // Current number of players
                                players[player.socketId].x = (playerNumberForSpawn -1) * SPAWN_OFFSET_X - ((MAX_PLAYERS-1)*SPAWN_OFFSET_X / 2); // Use MAX_PLAYERS for consistent spacing relative to full capacity
                                players[player.socketId].y = (playerNumberForSpawn -1) * SPAWN_OFFSET_Y - ((MAX_PLAYERS-1)*SPAWN_OFFSET_Y / 2);
                                players[player.socketId].angle = -Math.PI / 2;
                                
                                console.log(`[SERVER] Ship ${player.socketId} (${player.playerName}) RESPAWNED.`);
                                io.emit('player_respawned', players[player.socketId]); 
                            }
