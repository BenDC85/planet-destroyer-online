...
function setupSocketListeners() {
    ...
    socket.on('chunks_created', (newChunksData) => {
        ...
        newChunksData.forEach(chunkData => {
            ...
            // ---- START OF CODE TO ADD for new chunks ----
            newChunk.targetX = newChunk.x;       // Initially, target is where it currently is
            newChunk.targetY = newChunk.y;
            newChunk.targetAngle = newChunk.angle;
            newChunk.lastServerUpdateTime = Date.now(); // Or performance.now()
            // ---- END OF CODE TO ADD ----
    ...
    socket.on('chunks_update', (serverChunksData) => {
        ...
        serverChunksData.forEach(serverChunkData => {
            ...
            if (clientChunk) {
                clientChunk.targetX = serverChunkData.x;          // Store server's idea of X
                clientChunk.targetY = serverChunkData.y;          // Store server's idea of Y
                clientChunk.targetAngle = serverChunkData.angle;  // Store server's idea of Angle
                clientChunk.lastServerUpdateTime = Date.now();  // Remember when we got this target

                clientChunk.isActive = serverChunkData.isActive; // This can still be set directly
            }
        });
