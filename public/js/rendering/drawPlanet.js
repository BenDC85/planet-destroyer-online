...
export function renderPlanetState(ctx, planet) {
    ...
    if (!planet || (!planet.isBlackHole && (!planet.radius || planet.radius <= 0) && planet.isDestroyed )) {
        return;
    }
    ...
    if (!planet.isBlackHole && planet.radius <=0 && !planet.isDestroying && !planet.isBreakingUp) {
        return;
    }


    if (planet.isBlackHole) {
        // BH radius is the event horizon radius, set in stateModifiers
        ctx.beginPath(); 
...
function drawStaticCrateredPlanetInternal(ctx, planet) {
     if (!planet || !planet.radius || planet.radius <= 0 || !planet.type || !config.PLANET_TYPES[planet.type]) { return; }
    ...
    const radius = planet.radius; // Current visual radius
    const x = planet.x;
    const y = planet.y;
    
    const baseLineWidth = viz.GENERAL.BASE_LINE_WIDTH_ZOOM_INDEPENDENT;
    const rng = mulberry32(planet.id + 1); // Consistent PRNG per planet ID
...
function drawShatterCracksInternal(ctx, planet, progress) {
     if (!planet || !planet.lastImpactPoint || !planet.radius || planet.radius <= 0 || progress <= 0) return;
