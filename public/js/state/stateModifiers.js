...
export function setPersistentChunkDrift(isEnabled) {
    ...
    if (state && state.settings) {
        state.settings.persistentChunkDrift = !!isEnabled;
    }
}
...
export function setShipZoomAttractFactor(factor) {
    const state = getState();
    if (state && state.settings) {
        const numValue = parseFloat(factor);
        state.settings.shipZoomAttractFactor = Math.max(config.MIN_SHIP_ZOOM_ATTRACT_FACTOR, Math.min(config.MAX_SHIP_ZOOM_ATTRACT_FACTOR, isNaN(numValue) ? config.DEFAULT_SHIP_ZOOM_ATTRACT_FACTOR : numValue));
    }
}
...
export function setPlanetZoomAttractFactor(factor) {
    ...
    if (state && state.settings) {
        const numValue = parseFloat(factor);
        state.settings.planetZoomAttractFactor = Math.max(config.MIN_PLANET_ZOOM_ATTRACT_FACTOR, Math.min(config.MAX_PLANET_ZOOM_ATTRACT_FACTOR, isNaN(numValue) ? config.DEFAULT_PLANET_ZOOM_ATTRACT_FACTOR : numValue));
    }
}
...
export function setProjectileLaunchSpeed(internalSpeed) {
    const state = getState();
    if (state && state.settings) {
        const numValue = parseFloat(internalSpeed);
        const internalDefault = config.defaultProjectileSpeed * config.PROJECTILE_SPEED_HUD_SCALE_FACTOR;
        state.settings.projectileSpeed = Math.min(config.MAX_PROJECTILE_SPEED_INTERNAL, Math.max(config.MIN_PROJECTILE_SPEED_INTERNAL, isNaN(numValue) ? internalDefault : numValue));
    }
}
...
export function adjustProjectileLaunchSpeed(increase) {
    const state = getState();
    if (state && state.settings) {
        let currentInternalSpeed = state.settings.projectileSpeed;
        currentInternalSpeed += increase ? config.PROJECTILE_SPEED_STEP_INTERNAL : -config.PROJECTILE_SPEED_STEP_INTERNAL;
        setProjectileLaunchSpeed(currentInternalSpeed);
    }
}
...
export function setProjectileMass(mass) {
    const state = getState();
    if (state && state.settings) {
        const numValue = parseFloat(mass);
        state.settings.projectileMass = Math.max(config.minProjectileMass, isNaN(numValue) ? config.defaultProjectileMass : numValue);
    }
}
...
export function setBHParticleLifeFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        s.bhParticleLifeFactor = Math.max(0.1, isNaN(val) ? config.defaultBHParticleLifeFactor : val);
    }
}
export function setBHParticleSpeedFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        s.bhParticleSpeedFactor = Math.max(0.1, isNaN(val) ? config.defaultBHParticleSpeedFactor : val);
    }
}
export function setBHParticleSpawnRate(rate) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(rate, 10);
        s.bhParticleSpawnRate = Math.max(0, isNaN(val) ? config.defaultBHParticleSpawnRate : val);
    }
}
export function setBHMaxParticles(count) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(count, 10);
        s.bhMaxParticles = Math.max(config.minBHMaxParticles, isNaN(val) ? config.defaultBHMaxParticles : val);
    }
}
export function setBHSpawnRadiusMinFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        const minVal = Math.max(config.minBHSpawnRadiusFactor, isNaN(val) ? config.defaultBHSpawnRadiusMinFactor : val);
        s.bhSpawnRadiusMinFactor = Math.min(s.bhSpawnRadiusMaxFactor ?? Infinity, minVal);
    }
}
export function setBHSpawnRadiusMaxFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        const maxVal = Math.min(config.maxBHSpawnRadiusFactor, isNaN(val) ? config.defaultBHSpawnRadiusMaxFactor : val);
        s.bhSpawnRadiusMaxFactor = Math.max(s.bhSpawnRadiusMinFactor ?? 0, maxVal);
    }
}
export function setBHParticleMinSize(size) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(size);
        const minVal = Math.max(config.minBHParticleSize, isNaN(val) ? config.defaultBHParticleMinSize : val);
        s.bhParticleMinSize = Math.min(s.bhParticleMaxSize ?? Infinity, minVal);
    }
}
export function setBHParticleMaxSize(size) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(size);
        const maxVal = Math.min(config.maxBHParticleSize, isNaN(val) ? config.defaultBHParticleMaxSize : val);
        s.bhParticleMaxSize = Math.max(s.bhParticleMinSize ?? 0.1, maxVal);
    }
}
export function setBHInitialInwardFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        s.bhInitialInwardFactor = Math.max(config.minBHInitialInwardFactor, Math.min(config.maxBHInitialInwardFactor, isNaN(val) ? config.defaultBHInitialInwardFactor : val));
    }
}
export function setBHInitialAngularFactor(factor) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(factor);
        s.bhInitialAngularFactor = Math.max(config.minBHInitialAngularFactor, Math.min(config.maxBHInitialAngularFactor, isNaN(val) ? config.defaultBHInitialAngularFactor : val));
    }
}
export function setBHDragZoneMultiplier(multiplier) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(multiplier);
        s.bhDragZoneMultiplier = Math.max(config.minBHDragZoneMultiplier, Math.min(config.maxBHDragZoneMultiplier, isNaN(val) ? config.defaultBHDragZoneMultiplier : val));
    }
}
export function setBHDragCoefficientMax(coefficient) {
    const s = getState()?.settings;
    if (s) {
        const val = parseFloat(coefficient);
        s.bhDragCoefficientMax = Math.max(config.minBHDragCoefficientMax, Math.min(config.maxBHDragCoefficientMax, isNaN(val) ? config.defaultBHDragCoefficientMax : val));
    }
}
...
export function setPlanetGravityMultiplier(internalValue) {
    ...
    if (state && state.settings) {
        ...
        const internalDefault = config.defaultPlanetGravityMultiplier * config.PLANET_GRAVITY_HUD_SCALE_FACTOR;
        const internalMin = config.minPlanetGravityMultiplier * config.PLANET_GRAVITY_HUD_SCALE_FACTOR;
        state.settings.planetGravityMultiplier = Math.max(internalMin, isNaN(numValue) ? internalDefault : numValue);
        recalculateBHGravitationalConstant(state.settings);
    }
}
export function setChunkLifespan(frames) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(frames, 10);
        s.chunkLifespanFrames = Math.max(config.minChunkLifespan, isNaN(val) ? config.defaultChunkLifespan : val);
    }
}
export function setChunkMaxSpeed(speed) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(speed, 10);
        s.chunkMaxSpeedThreshold = Math.max(config.minChunkMaxSpeed, Math.min(config.maxChunkMaxSpeed, isNaN(val) ? config.defaultChunkMaxSpeed : val));
    }
}
export function setCoreExplosionDuration(frames) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(frames, 10);
        s.coreExplosionDuration = Math.max(config.minCoreExplosionDuration, Math.min(config.maxCoreExplosionDuration, isNaN(val) ? config.defaultCoreExplosionDuration : val));
    }
}
export function setCoreImplosionDuration(frames) {
    const s = getState()?.settings;
    if (s) {
        const val = parseInt(frames, 10);
        s.coreImplosionDuration = Math.max(config.minCoreImplosionDuration, Math.min(config.maxCoreImplosionDuration, isNaN(val) ? config.defaultCoreImplosionDuration : val));
    }
}
export function setCraterScalingC(value) {
    const state = getState();
    if (state?.settings) {
        const numValue = parseFloat(value);
        state.settings.craterScalingC = Math.max(config.minCraterScalingC, isNaN(numValue) ? config.defaultCraterScalingC : numValue);
    }
}
export function setKeToMassEjectEta(value) {
    const state = getState();
    if (state?.settings) {
        const numValue = parseFloat(value);
        state.settings.keToMassEjectEta = Math.max(config.minKEToMassEjectEta, isNaN(numValue) ? config.defaultKEToMassEjectEta : numValue);
    }
}
export function setBHEnergyMultiplier(value) {
    const state = getState();
    if (state?.settings) {
        const numValue = parseFloat(value);
        state.settings.bhEnergyMultiplier = Math.max(config.minBHEnergyMultiplier, isNaN(numValue) ? config.defaultBHEnergyMultiplier : numValue);
    }
}
...
export function setSetupPlanetCount(count) {
    ...
    if (s) {
        const val = parseInt(count, 10);
        s.planetCount = Math.max(config.minPlanetCount, Math.min(config.maxPlanetCount, isNaN(val) ? config.defaultPlanetCount : val));
    }
}
export function setBHGravityFactor(factor) {
    ...
    if (state && state.settings) {
        const numValue = parseFloat(factor);
        state.settings.bhGravityFactor = Math.max(config.minBHGravityFactor, Math.min(config.maxBHGravityFactor, isNaN(numValue) ? config.defaultBHGravityFactor : numValue));
        recalculateBHGravitationalConstant(state.settings);
    }
}
function recalculateBHGravitationalConstant(settings) {
    if (!settings || typeof settings.G !== 'number' || typeof settings.referencePlanetMassForBHFactor !== 'number' || typeof settings.planetGravityMultiplier !== 'number' || typeof settings.bhGravityFactor !== 'number') {
        return;
    }
