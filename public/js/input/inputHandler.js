...
import { socket } from '../network.js'; // **** NEW: Import socket for sending reset request ****
...
export function setupInputListeners(canvasElement) {
    ...
    if (resetButton) { resetButton.addEventListener('click', handleReset); } // Listener remains, handler changes
...
function handleKeyDown(event) {
    ...
    if (keyState.hasOwnProperty(event.code)) {
        ...
        if (!keyState[event.code]) { 
            ...
            else if (event.code === 'Space') stateModifiers.fireShipProjectile(); 
        }
    }
}
...
function handleReset() {
    // **** MODIFIED: Now sends a request to the server ****
    if (!canvas) { console.error("Reset failed: Canvas not found."); return; }
    
    // Optionally, gather any settings the client wants to suggest for the reset (e.g., planet count)
    // For now, let's assume the server uses its default planet count or a fixed number for resets.
    // If we want to send client-preferred settings for reset:
    // const settingsToSuggest = {
    //     planetCount: parseInt(planetCountInput?.value, 10) || config.defaultPlanetCount 
    //     // Add other settings if they should influence server-side generation
    // };

    console.log("InputHandler: Requesting server to reset world...");
    if (socket && socket.connected) {
        socket.emit('request_world_reset', {}); // Send empty object or settingsToSuggest
    } else {
        console.error("Cannot send world reset request: Socket not connected.");
        // Optionally, attempt a local reset as fallback, or just inform user
        // For now, we'll rely on server connection for resets.
    }

    // The old local reset logic is removed. Client will re-initialize based on server's 'world_reset_data' event.
}
