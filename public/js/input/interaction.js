...
export function processClick(secondWorldClickCoords, currentState) {
    ...
    // In multiplayer, this action would likely be sent to the server.
    const firstWorldClick = currentState.firstClickCoords;
    stateModifiers.setClickState('idle'); // Reset click state
    if (!firstWorldClick || !secondWorldClickCoords) return;

    const p1 = firstWorldClick;
    const p2 = secondWorldClickCoords;
    const segmentLengthSq = utils.distanceSq(p1, p2);

    if (segmentLengthSq < MIN_INTERACTION_SEGMENT_LENGTH_SQ) return; // Click-drag too short

    let firstHit = null; 

    currentState.planets.forEach(planet => {
        // Only interact with active, non-destroying, non-BH planets
        if (!planet.isBreakingUp && !planet.isDestroying && !planet.isBlackHole && 
            planet.chunkGracePeriodFrame === 0 && (planet.massKg ?? 0) > 0) {
            
            const entryPointInfo = findInteractionImpactInternal(p1, p2, planet); 
            if (entryPointInfo) {
                if (firstHit === null || entryPointInfo.t < firstHit.t - INTERACTION_IMPACT_T_PRECISION) {
                    firstHit = {
                        t: entryPointInfo.t,
                        planet: planet,
                        point: entryPointInfo.point
                    };
                }
            }
        }
    ...
    if (firstHit) {
        const targetPlanet = firstHit.planet;
        // Double-check planet state before applying damage (it might have changed mid-process)
        if (targetPlanet.isBreakingUp || targetPlanet.isDestroying || targetPlanet.isBlackHole || 
            targetPlanet.chunkGracePeriodFrame > 0 || (targetPlanet.massKg ?? 0) <= 0) {
             return; 
        }
        const impactPoint = firstHit.point;
        const massBeforeCrater = targetPlanet.massKg;
        
        // For direct click, crater radius comes from settings.baseDamageRadius, mass ejected is a fraction of planet's current mass
        // This is different from projectile impact which calculates based on KE.
        // Let's make direct click less powerful than a projectile, or define its own mass ejection rules.
        // For now, let's use a placeholder mass ejection based on a fraction of the planet's current mass or a fixed small amount.
        const directClickMassEject = targetPlanet.massKg * 0.01; // E.g., 1% of current mass
        const directClickKE = 0; // Direct clicks don't have KE in the same way projectiles do

        const newMass = stateModifiers.addCrater(targetPlanet, impactPoint.x, impactPoint.y, null, directClickMassEject, directClickKE);
        
        const destructionThresholdFactor = config.DIRECT_CLICK_DESTRUCTION_MASS_THRESHOLD_FACTOR; 
        const destructionThreshold = destructionThresholdFactor * targetPlanet.originalMassKg;
        
        if ((newMass <= destructionThreshold && massBeforeCrater > destructionThreshold) ||
            (newMass <= 0 && massBeforeCrater > 0)) {
            targetPlanet.willBecomeBlackHole = false; // Direct clicks usually don't create BHs unless specifically designed
            stateModifiers.triggerDestructionSequence(targetPlanet, impactPoint.x, impactPoint.y);
        }
    }
}
...
function findInteractionImpactInternal(p1, p2, planet) {
    ...
    for (let i = 0; i < uniqueEvents.length; i++) {
        ...
        if (i < uniqueEvents.length - 1) { // Test midpoint of [currentEvent, nextEvent]
            ...
            // If only one event and it's the start point, and it's not inside, no impact from this.
            // If it's an intersection point at t < 1.0, we need to check its status.
        }
